...
export const createNewEstilo = async (req, res) => {
  ...
  // validating
  /*
  if ( !estilo || !nombreestilo) {
    return res.status(400).json({
      msg: 'Requerimiento incorrecto. Por favor proporciona estilo y nombre del estilo',
    });
  }
  */
  
  //if (!req.body.estilo || !req.body.nombre) {
   // res.status(400).send('Requerimiento incorrecto. Por favor proporciona estilo y nombre del estilo');
   // res.status(400);
  // console.log('pasa');
  // res.send('mensaje');
  // console.log('si pasa');
